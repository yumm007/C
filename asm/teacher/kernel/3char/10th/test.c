...
unsigned int test_poll (struct file *filp, struct poll_table_struct *table)
{
	...
	poll_wait(filp, &dev->rq, table);			
	poll_wait(filp, &dev->wq, table);			

	if(dev->cur_size > 0){	//readalbe
		mask |= POLLIN | POLLRDNORM;	
	}
	if(dev->cur_size < DEV_SIZE){//writeable
...
{
	struct test_dev_t *dev = (struct test_dev_t *)filp->private_data;
	return fasync_helper(fd, filp, on, &dev->fa);	
}
...
int test_init(void)
{
	...
	//init
	test.cur_size = 0;
	init_waitqueue_head(&test.q);
	init_waitqueue_head(&test.rq);
