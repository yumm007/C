...
static inline bool get_down(unsigned long data0, unsigned long data1)
{
	...
	return (!((data0 & S3C2410_ADCDAT0_UPDOWN)>>15) &&
		!((data1 & S3C2410_ADCDAT0_UPDOWN)>>15));
}
...
static void touch_timer_fire(unsigned long data)
{
	...
	if (down) {
		if (ts.count == (1 << ts.shift)) {
			...
			printk(KERN_WARNING "%s: X=%lu, Y=%lu, count=%d\n",  __func__, ts.xp, ts.yp, ts.count);
...
static void s3c24xx_ts_select(struct s3c_adc_client *client, unsigned select)
{
	if (select) {
		...
		mod_timer(&touch_timer, jiffies+30);
...
static int __devinit s3c2410ts_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(ts.input, ABS_X, 0, 0x3FF, 0, 0); //默认值和初始值
	input_set_abs_params(ts.input, ABS_Y, 0, 0x3FF, 0, 0);
	input_set_abs_params(ts.input, ABS_PRESSURE, 0, 1, 0, 0); //1有压力，初始为0

	ts.input->name = "s3c2410 TouchScreen";
