...
static void rel8_decode(const unsigned char *data, int len, unsigned char *out) {
	int i, j;
	for (i = 0; i < len;) {
		switch (data[i]) {
			case 0x00:
...
int main(void) {
	...
	printf("encode %ld bytes to %d bytes\n", sizeof(gImage_1bit_arr), len);
	rel8_decode(imgcode, len, imgdecode);
	printf("decode %s.\n", memcmp(gImage_1bit_arr, imgdecode, len) == 0 ? "succ": "failed");

	len = rel8_encode(gImage_2bit_arr, sizeof(gImage_2bit_arr), imgcode);
	printf("encode %ld bytes to %d bytes\n", sizeof(gImage_2bit_arr), len);
	rel8_decode(imgcode, len, imgdecode);
	printf("decode %s.\n", memcmp(gImage_2bit_arr, imgdecode, len) == 0 ? "succ": "failed");
