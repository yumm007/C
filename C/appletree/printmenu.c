...
int main(void) 
{
   ...
   if ((sturoot = load_tree()) == NULL){
      printf("load db faild.");
		any_key();
      if ((sturoot = create_boxtree()) == NULL) {
         printf("create date faild.\n");
         return -1; 
      }   
   }   
		...
		case '1': case 'F': case 'f':
				printf("Please enter the search condition: [ID],[Name],[Scope]\n");
				{
					...
					if ((tmp = calloc(sizeof(*tmp) + MAXNAME,sizeof(char))) == NULL) 
						...
						printf("realloc error!\n");
					else if ((find = find_box(sturoot, tmp, cmp_id)) == NULL)
						printf("Record is not exist!");
					else
						print_node(find);
					free(tmp);
		...
		case '2': case 'I': case 'i':
				...
				printf("Please enter the new record: 'ID,Name,Scope:'\n");
				{
					...
					if ((tmp = calloc(sizeof(*tmp) + MAXNAME,sizeof(char))) == NULL)
						...
						printf("Input Format wrong!\n");
					else if (put_box(sturoot, tmp, offsetof(struct stu, name) + strlen(tmp->name) + 1, cmp_id) == -1)
						printf("Insert faild!");
					else
						printf("Insert OK!");
					free(tmp);
		...
		case '3': case 'D': case 'd':
				printf("Please enter the condition : [ID],[Name],[Scope]\n");
				{
					...
					if ((tmp = calloc(sizeof(*tmp) + MAXNAME,sizeof(char))) == NULL)
						...
						printf("Input Format wrong!\n");
					del_box(sturoot,tmp, cmp_id);
					free(tmp);
				}
				any_key();
				break;
		case '4': case 'L': case 'l':
				travel_tree(sturoot,print_node);
				any_key();
				break;
