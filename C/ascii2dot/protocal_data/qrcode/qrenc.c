...
static void writeBUF(QRcode *qrcode, uint8_t *qrbuf)
{
	memcpy(qrbuf, &qrcode->width, 4);
	memcpy(qrbuf + 4, qrcode->data, (qrcode->width) * (qrcode->width));
}
...
int qrcode_main(int argc, char **argv, uint8_t *qr_buf)
{
	int opt, lindex = -1;
	char *outfile = NULL;
