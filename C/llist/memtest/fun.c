...
struct stu *read_record(void){
     struct stu *pstu = request_mem(memllist,sizeof(struct stu)); /* 申请空间并初始化*/
	  if (pstu == NULL)
			return NULL;
     if (getline(pstu) == 0) {
	  		free_mem(memllist, pstu);
	  		return NULL;
     }
     return pstu;
}
...
int cmp_id(const void *data1, const void *data2) {
	struct stu **p1 = (struct stu **)data1;
	struct stu **p2 = (struct stu **)data2;
	return ( (*p1)->id - (*p2)->id );
}
