...
void update(LLIST *sturoot) {
	struct stu *tmp, *newdata, *find;
			...
			if ((tmp = request_mem(memllist, (sizeof(*tmp) + MAXNAME)*sizeof(char))) == NULL) {
				printf("request_mem error!");
				return;
			}
			if (getline(tmp) == 0) {
			...
			if ((tmp = resize_mem(memllist,tmp, offsetof(struct stu, name) + strlen(tmp->name) + 1)) == NULL) {
				printf("resize_mem error!");
				free_mem(memllist, tmp);
				return;
			}
			...
			if ((newdata = request_mem(memllist, (sizeof(*newdata) + MAXNAME)*sizeof(char))) == NULL) {
				printf("request mem error!");
				return;
			}
			printf("Please enter the new data: [ID],[Name],[Scope]\n");
			if (getline(newdata) == 0) { 
				free_mem(memllist, newdata);
				printf("Input Format wrong!");
				return;
			}
	      ...
	      if (newdata->id == -1)
			...
			if ((newdata = resize_mem(memllist,newdata, offsetof(struct stu, name) + strlen(newdata->name) + 1)) == NULL)
				printf("resize_mem error!\n");
...
int main(void) 
{
	...
	if ((tmp = malloc(5048000)) == NULL) {
		printf("malloc for memllist faild\n");
		return 0;
	}

	if ((memllist = init_mem(tmp, 4096000)) == NULL) {
		...
		case '1': case 'F': case 'f':
				printf("Please enter the search condition: [ID],[Name],[Scope]\n");
				{
					...
					if ((tmp = request_mem(memllist,(sizeof(*tmp) + MAXNAME) *sizeof(char))) == NULL) 
						...
						printf("Input Format wrong!\n");
					else if ((tmp = (struct stu *)resize_mem(memllist, tmp, offsetof(struct stu, name) + strlen(tmp->name) + 1)) == NULL)
						printf("resize_mem error!\n");
