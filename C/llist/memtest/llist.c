...
extern MEMLLIST *memllist;

LLIST *create_llist(void) {
	LLIST *llist;

	if((llist = request_mem(memllist,sizeof(LLIST))) == NULL)
		return NULL;
...
int insert_llist(LLIST *llist, const void *data, const size_t size, const int mode) {
	...
	if ((new = request_mem(memllist, sizeof(struct list_node))) == NULL)
		return -1;
	if ((tmp = request_mem(memllist,size)) == NULL) {
		free_mem(memllist, new);
		return -1;
	}
...
void erase(LLIST *llist) {
	...
	free_mem(memllist, cur->prev);
	free_mem(memllist, llist);
}
...
LLIST *load_llist(const char *filepath){
	...
	for (i = 0; !faild && i < num; i++) {
		if (fread(&size, sizeof(size_t) ,1, fp) < 1)
			faild = 1;
		else if ((data = resize_mem(memllist, data, size)) == NULL) 
			faild = 1;
...
int llist_sort(LLIST *llist, int (*cmp)(const void *, const void*), void (*op)(void * data)) {
	...
	num = llist->num;
	if ((base =  request_mem(memllist, num * sizeof(void *))) == NULL)
		return -1;
	arr = base;
