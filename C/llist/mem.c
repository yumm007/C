...
LLIST *init_mem(void *addr, size_t size) {
	...
	llist = (LLIST *) re_size_add(addr, &size);
...
void delete_node(LLIST *llist, struct free_mem_node *cur) {
...
void *request_mem(LLIST *llist, size_t size) {
	...
	if (cur->size < sizeof(*cur) - sizeof(size_t)) {
		cur->prev->next = cur->next;
		cur->next->prev = cur->prev;
		llist->num--;
		ret = (char *)cur;
...
static void insert_free_node(LLIST *llist, struct free_mem_node *pos) {
	struct free_mem_node *cur;
	ssize_t i;
